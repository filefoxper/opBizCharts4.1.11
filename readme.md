...
使用 webpack module 的 rules 建立一个强行编译待优化库的 rule ，如果该库含有 es/esm 等模块化包，可以通过 webpack resolve 的 alias 别名功能，把引用映射至 es/esm 包，这样 webpack + babel 可以在编译时按我们需求选择单一有效的目录了，如：
